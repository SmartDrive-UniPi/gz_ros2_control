    ...
    sim::EventManager &)
{
    ...
    // Initialize ResourceManager with required arguments
    auto clock_interface = this->dataPtr->node_->get_node_clock_interface();
    auto logging_interface = this->dataPtr->node_->get_node_logging_interface();
    ...
    // Removed the load_urdf call as it's no longer available
    /*
    try {
